...
### Tablas Principales descripción:
...
1.  **usuario**: `id`, `username`, `email`, `password`, `estado`, `fecha_creacion`, `avatar`.
2.  **rol**: `id`, `nombre`, `descripcion`.
3.  **usuario_rol**: `id`, `id_user`, `id_rol`.
4.  **area**: `id`, `nombre`, `descripcion`, `imagen` (Ministerios o departamentos de la iglesia).
5.  **puesto**: `id`, `id_user`, `id_area`, `id_rol`, `requiere_confirmacion`, `estado`.
6.  **blog**: `id`, `id_user`, `titulo`, `contenido`, `fecha_creacion`, `estado`, `imagen`.
7.  **comentario**: `id`, `id_user`, `id_blog`, `id_padre`, `contenido`, `imagen`, `fecha_creacion`, `fecha_actualizacion`, `estado`.
8.  **reaccion**: `id`, `emoji`.
9.  **reaccion_blog**: `id`, `id_user`, `id_blog`, `id_reaccion`.
10. **evento**: `id`, `id_area`, `titulo`, `descripcion`, `fecha_inicio`, `fecha_fin`, `aforo_max`, `estado`.
11. **reserva_evento**: `id`, `id_user`, `id_evento`, `estado`, `fecha_reserva`.
12. **oracion**: `id`, `id_user`, `titulo`, `contenido`, `fecha_creacion`, `duracion_dias`, `estado`.
13. **oracion_recordatorio**: `id`, `id_user`, `id_oracion`.
14. **tag**: `id`, `nombre`, `tipo`.
15. **relaciones_tags**: `id_blog`, `id_tag`, `id_oracion`, `id_tag`, `id_evento`, `id_tag`.
...
-   **Servicio Frontend (frontend)**:
    -   Construido en dos etapas (etapa de build con Node y etapa de servidor con Nginx).
    -   Sirve la aplicación React/Vite de forma optimizada.
    -   Expone el puerto `80`.

### Pasos para el Despliegue Local:
1.  Clonar el repositorio.
2.  Configurar el archivo `.env` tomando como referencia el archivo `.env.example`.
...
### 9.4 Perfil y Área Personal
Los usuarios disponen de un panel privado para gestionar su identidad, visualizar sus actividades y realizar el seguimiento de sus peticiones.
